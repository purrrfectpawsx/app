...
This section documents proven, reusable component patterns established during implementation. These patterns ensure consistency across the codebase and accelerate development by providing tested solutions for common scenarios.

> **Epic 2 Retrospective Recommendation:** Document these patterns to maintain consistency and accelerate Epic 3+ development.

### Pattern 1: Form Components with React Hook Form + Zod

**Use Case:** Any user input form (create/edit operations)

**Established In:** Story 2.1 (CreatePetForm), Story 2.4 (EditPetForm)

**Pattern:** Combine Zod schema for validation with React Hook Form for optimal performance and type safety.
...
**Benefits:** Single source of truth, TypeScript safety, real-time validation

**Reuse in:** Health records, expenses, reminders, documents (Epic 3+)

---

### Pattern 2: Photo Upload with Compression

**Use Case:** Image upload with quality optimization

**Established In:** Story 2.1 (PetPhotoUpload)

**Pattern:** Compress images client-side before upload using `browser-image-compression`
...
**Benefits:** 50-70% file size reduction, faster uploads, lower storage costs

**Reuse in:** Health record attachments (3.9), Expense receipts (4.10)

---

### Pattern 3: Supabase Storage Image Transforms

**Use Case:** Display images at different sizes for different contexts

**Established In:** Story 2.2 (Grid thumbnails), Story 2.3 (Detail views)

**Pattern:** Use Supabase transform API for responsive images

```typescript
// Grid: 300x300
const { data } = supabase.storage.from('pets-photos').getPublicUrl(path, {
...
**Benefits:** Reduced bandwidth, faster loads, server-side transformation

**Sizing Guidelines:** Grid/list: 300x300 | Detail: 600x600 | Full: No transform

---

### Pattern 4: RLS Policies for Tier Enforcement

**Use Case:** Enforce business logic (tier limits) at database level

**Established In:** Story 2.6 (Free tier 1 pet limit)

**Pattern:** Combine frontend UX checks with backend RLS enforcement

```sql
CREATE POLICY "tier_limit_pets" ON pets FOR INSERT
WITH CHECK (
  auth.uid() = user_id AND (
    (SELECT subscription_tier FROM profiles WHERE id = auth.uid()) = 'premium'
    OR (SELECT COUNT(*) FROM pets WHERE user_id = auth.uid()) < 1
  )
);
```

**Benefits:** Frontend (UX), Backend (security), business logic in database

**Reuse in:** All tier-limited features (health records, expenses, reminders, storage)

---

### Pattern 5: Component Composition

**Use Case:** Breaking complex pages into focused components

**Established In:** Story 2.3 (PetDetailPage → PetInfoCard + PetStats + Tabs)

**Pattern:** Page orchestrator delegates to focused child components

**Benefits:** Single responsibility, easier testing, better maintainability

**Reuse in:** HealthTimelinePage, ExpenseDashboard, RemindersList

---

### Pattern 6: Cascade Deletion via Foreign Keys

**Use Case:** Auto-delete related records when parent deleted

**Established In:** Story 2.5 (Delete pet → cascade to all child tables)

**Pattern:** Use ON DELETE CASCADE in foreign key constraints
...
**Benefits:** No complex app logic, database integrity, single DELETE query

**Important:** Manually delete Supabase Storage files (not cascaded)

**Reuse in:** All parent-child relationships

---

### Pattern 7: Shared Utility Functions

**Use Case:** Consistent calculations across components

**Established In:** Story 2.2, 2.3 (`calculatePetAge` used in multiple components)

**Pattern:** Extract common calculations to utilities
...
**Use Case:** Better UX during data fetching

**Established In:** Story 2.2 (PetsGrid), Story 2.3 (PetDetailPage)

**Pattern:** Show skeleton UI matching final layout

**Benefits:** Prevents layout shift, better perceived performance

**Reuse in:** All data-heavy pages

---

### Pattern 9: Empty States with CTAs

**Use Case:** Helpful UI when no data exists

**Established In:** Story 2.2 (EmptyPetsState)

**Pattern:** Icon + helpful message + clear CTA button

**Guidelines:** Friendly heading, helpful description, centered layout, CTA button

**Reuse in:** Empty health timeline, expenses, reminders, documents

---

### Pattern 10: Confirmation Dialogs

**Use Case:** Prevent accidental destructive actions

**Established In:** Story 2.5 (DeletePetDialog)

**Pattern:** Use AlertDialog with clear consequences, destructive styling

**Guidelines:** Red button, explain what will be deleted, mention if irreversible

**Reuse in:** All delete operations

---

### Pattern 11: Toast Notifications

**Use Case:** User feedback for actions

**Established In:** All Epic 2 stories

**Pattern:** Success/error toasts for all CRUD operations
...
**Guidelines:** Short title, optional description, auto-dismiss after 5s

**Reuse in:** All CRUD operations

---

### Pattern 12: Tier Limit UI Components

**Use Case:** Inform users about limits and prompt upgrades

**Established In:** Story 2.6 (TierLimitBanner, UsageIndicator, UpgradePromptDialog)

**Components:**
- Usage Indicator: "X/Y used (Free plan)"
- Tier Limit Banner: Shows when limit reached
- Upgrade Prompt Dialog: Explains benefits, links to pricing

**Reuse in:** All tier-limited features

---

### Pattern 13: Responsive Grid Layouts

**Use Case:** Grid displays adapting to screen size

**Established In:** Story 2.2 (PetsGrid)

**Pattern:** Tailwind responsive grid classes
...
**Reuse in:** Health timeline, expense list, document library

---

## Pattern Application Checklist

Use this checklist when implementing new features in Epic 3+:
...
- [ ] Proper storage path
- [ ] RLS policies
- [ ] Transform URLs for sizes

---

_Component Patterns Library added: 2025-11-15 (Epic 2 Retrospective Action Item)_
